/*- 
 * Classname:             AddBookFC.java 
 * 
 * Version information:   1.1
 * 
 * Date:                  02/08/2014 - 17:05:10 
 * 
...
/**
 * janela para adição livro FC
 *
 * @see
 * @author Jonas Mayer (jmayer13@hotmail.com)
 */
public class AddBookFCFrame implements AddBookInterface {
     ...
     */
    public AddBookFCFrame(int codeSugered, String titleBox) {
        this.titleBox = titleBox;
        title = "Adicionar livro - " + Category.FC.getName();
        createView();
     ...
     */
    public AddBookFCFrame(String titleBox, BookFC bookFC) {
        this.bookFC = bookFC;
        this.titleBox = titleBox;
        title = "Editar livro - " + Category.FC.getName();
        createView();
        fillsView();
    ...
    private void fillsView() {
        codeField.setText("" + bookFC.getCodeBookSpecific());
        courseField.setText(bookFC.getTitleBook());
        String year = bookFC.getYear();
        yearNoneRadioButton.setSelected(false);
        if (year == null) {
            yearNoneRadioButton.setSelected(true);
        ...
        observationField.setText(bookFC.getObservationBook());
    ...
    public BookFC getBook() {
        ...
        try {
            bookFC.setCodeBookSpecific(Integer.valueOf(codeField.getText()));
            bookFC.setTitleBook(courseField.getText());
            if (intervalRadioButton.isSelected()) {
                bookFC.setYear(firstIntervalField.getText() + "-" + secondIntervalField.getText());
            } else if (yearRadioButton.isSelected()) {
                bookFC.setYear(yearField.getText());
            }
            bookFC.setObservationBook(observationField.getText());
            return bookFC;
    ...
    private Color getColor() {
        ...
        return colorConverter.getColor(PreferencesManager.KEY_COLOR_BOOK_FC);
    }//fim do método getColor

    //método para testes
    public static void main(String args[]) {
        uni.uri.salmox.view.AddBookFCFrame addBookFCFrame = new uni.uri.salmox.view.AddBookFCFrame(0, "test");
        List<String> list = new ArrayList();
        list.add("teste");
        addBookFCFrame.setCourseList(list);
    }
}//fim da classe AddBookFC 
