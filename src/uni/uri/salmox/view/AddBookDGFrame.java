/*- 
 * Classname:             AddBookDG.java 
 * 
 * Version information:   1.1 
 * 
 * Date:                  02/08/2014 - 17:11:27 
 * 
...
/**
 * janela para adição livro DG
 *
 * @see
 * @author Jonas Mayer (jmayer13@hotmail.com)
 */
public class AddBookDGFrame implements AddBookInterface {
     ...
     */
    public AddBookDGFrame(int codeSugered, String titleBox) {
        this.titleBox = titleBox;
        title = "Adicionar livro - " + Category.DG.getName();
        createView();
     ...
     */
    public AddBookDGFrame(String titleBox, BookDG bookDG) {
        this.bookDG = bookDG;
        this.titleBox = titleBox;
        title = "Editar livro - " + Category.DG.getName();
        createView();
        fillsView();
...
//define evento
    ...
    private void fillsView() {
        codeField.setText("" + bookDG.getCodeBookSpecific());
        courseField.setText(bookDG.getTitleBook());
        String year = bookDG.getYear();
        yearNoneRadioButton.setSelected(false);
        if (year == null) {
            yearNoneRadioButton.setSelected(true);
    ...
    public BookDG getBook() {
        ...
        try {
            bookDG.setCodeBookSpecific(Integer.valueOf(codeField.getText()));
            bookDG.setTitleBook(courseField.getText());
            if (intervalRadioButton.isSelected()) {
                bookDG.setYear(firstIntervalField.getText() + "-" + secondIntervalField.getText());
            } else if (yearRadioButton.isSelected()) {
                bookDG.setYear(yearField.getText());
            }
            bookDG.setObservationBook(observationField.getText());
            return bookDG;
    ...
    private Color getColor() {
        ...
        return colorConverter.getColor(PreferencesManager.KEY_COLOR_BOOK_DG);
    }//fim do método getColor

    //método para testes
    public static void main(String args[]) {
        uni.uri.salmox.view.AddBookDGFrame addBookDGFrame = new uni.uri.salmox.view.AddBookDGFrame(0, "test");
        List<String> list = new ArrayList();
        list.add("teste");
        addBookDGFrame.setCourseList(list);
    }
}//fim da classe AddBookDG 
