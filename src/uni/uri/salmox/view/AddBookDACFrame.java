/*- 
 * Classname:             AddBookDAC.java 
 * 
 * Version information:   1.1
 * 
 * Date:                  02/08/2014 - 17:06:00 
 * 
...
/**
 * janela para adição livro diretora academica
 *
 * @see
 * @author Jonas Mayer (jmayer13@hotmail.com)
 */
public class AddBookDACFrame implements AddBookInterface {
     ...
     */
    public AddBookDACFrame(int codeSugered, String titleBox) {
        this.titleBox = titleBox;
        title = "Adicionar livro - " + Category.DAC.getName();
        createView();
     ...
     */
    public AddBookDACFrame(String titleBox, BookDAC bookDAC) {
        this.bookDAC = bookDAC;
        this.titleBox = titleBox;
        title = "Editar livro - " + Category.DAC.getName();
        createView();
        fillsView();
    ...
    private void fillsView() {
        codeField.setText("" + bookDAC.getCodeBookSpecific());
        courseField.setText(bookDAC.getTitleBook());
        String year = bookDAC.getYear();
        yearNoneRadioButton.setSelected(false);
        if (year == null) {
            yearNoneRadioButton.setSelected(true);
        ...
        observationField.setText(bookDAC.getObservationBook());
    ...
    public BookDAC getBook() {
        ...
        try {
            bookDAC.setCodeBookSpecific(Integer.valueOf(codeField.getText()));
            bookDAC.setTitleBook(courseField.getText());
            if (intervalRadioButton.isSelected()) {
                bookDAC.setYear(firstIntervalField.getText() + "-" + secondIntervalField.getText());
            } else if (yearRadioButton.isSelected()) {
                bookDAC.setYear(yearField.getText());
            }
            bookDAC.setObservationBook(observationField.getText());
            return bookDAC;
    ...
    private Color getColor() {
        ...
        return colorConverter.getColor(PreferencesManager.KEY_COLOR_BOOK_DAC);
    }//fim do método getColor

    //método para testes
    public static void main(String args[]) {
        uni.uri.salmox.view.AddBookDACFrame addBookDACFrame = new uni.uri.salmox.view.AddBookDACFrame(0, "test");
        List<String> list = new ArrayList();
        list.add("teste");
        addBookDACFrame.setCourseList(list);
    }
}//fim da classe AddBookDAC 
